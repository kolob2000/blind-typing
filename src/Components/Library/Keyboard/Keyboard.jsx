...
export const Keyboard = ({ char }) => {
  const isShifted = (character) => {
    const shiftedCharacters =
      'Ё!"№;%:?*()_+/ЪХЗЩШГНЕКУЦЙФЫВАПРОЛДЖЭ,' +
      'ЮБЬТИМСЧЯ~!@#$%^&*()_+}{POIUYTREWQASDFGHJKL:"?><MNBVCXZ';
    return shiftedCharacters.includes(character)
      ? " " + styles.highlighted
      : "";
  };
  ...
  class ButtonProps {
    constructor(text = "", className = "", isWord = false, isWin = false) {
      this.text = text;
  ...
  const rowFour = [
    new ButtonProps(
      "shift",
      styles.keyboard__button_lshift + isShifted(char),
      true
    ),
    new ButtonProps("ZЯ"),
    new ButtonProps("XЧ"),
  ...
  return (
    <Container
      className={
        "w-auto d-flex flex-column flex-nowrap mt-5 " + styles.keyboard
      }
    >
