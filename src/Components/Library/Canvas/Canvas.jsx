...
export const Canvas = ({text, position, index}) => {
    ...
    useEffect(() => {
        ...
        (position === "center") && (canvas.current.width = context.measureText(text[index]).width);
        ...
        context.clearRect(0, 0, canvas.current.width, canvas.current.height)
            ...
            context.fillText(
                position === "start" ? text.slice(0, index)
                    : position === "end" ? text.slice(index + 1, text.length)
                        : text[index]
                , position === "start" ? canvas.current.width : 0
                , textY)
        }
    }, [index])
    return <Col className={"col-auto p-0"}>
        <canvas ref={canvas} style={{
            filter: position === "start" ? "blur(2px)" : ""
        }}
        >
        </canvas>
    </Col>
